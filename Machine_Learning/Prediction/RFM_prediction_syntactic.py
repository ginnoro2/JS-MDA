...
lr_model = joblib.load('random_forest_syntactic.pkl')
pca = joblib.load('pca_syntactic.pkl')
feature_encoder = joblib.load('label_encoder_Features.pkl')

# Load the new syntactic data (e.g., 'test.csv')
filename = 'mtest.csv'
dtype_spec = {
    'Filename': str,
    'FeatureID': int,
...
# Encode the Feature using the same encoder
print("Encoding Feature in the new data...")
new_data['Feature'] = feature_encoder.transform(new_data['Feature'])

# Prepare the feature set (X) - In this case, only Feature is used since PCA is applied to it
...
# Check if PCA output matches the model input dimensions
if X_new_pca.shape[1] != lr_model.n_features_in_:
    raise ValueError(f"Unexpected number of features after PCA: got {X_new_pca.shape[1]}, expected {lr_model.n_features_in_}")
